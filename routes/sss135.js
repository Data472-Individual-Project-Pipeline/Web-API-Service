...
module.exports = (pool, logger) => {
   ...
   * tags:
   *   - name: Shahron Shaji ~
   *     description: API endpoints for Shahron Shaji ~
   */

  // Endpoint for SSS135: prisoner_age_group
  /**
   * @swagger
   * /sss135/prisoner_age_group:
   *   get:
   *     summary: Retrieve data from Shahron Shaji ~
   *     tags: [Shahron Shaji ~]
   *     responses:
   *       200:
   ...
   */
  router.get('/sss135/prisoner_age_group', async (req, res) => {
    try {
      const result = await pool.query('SELECT * FROM prisoner_age_group WHERE owner = $1', ['sss135']);
   ...
   *                   description: Error message
   */
    router.get('/sss135/prisoner_ethnicity', async (req, res) => {
        try {
          const result = await pool.query('SELECT * FROM prisoner_ethnicity WHERE owner = $1', ['sss135']);
          res.json(result.rows);
        } catch (err) {
    ...
    // Endpoint for Shahron Shaji ~: prisoner_offence_type
      /**
       * @swagger
       * /sss135/prisoner_offence_type:
       *   get:
       *     summary: Retrieve data from Shahron Shaji ~
       *     tags: [Shahron Shaji ~]
       *     responses:
       ...
       *                   description: Error message
       */
      router.get('/sss135/prisoner_offence_type', async (req, res) => {
        try {
          const result = await pool.query('SELECT * FROM prisoner_offence_type WHERE owner = $1', ['sss135']);
          res.json(result.rows);
        } catch (err) {
  ...
  // Endpoint for SSS135: prisoner_population
  /**
   * @swagger
   * /sss135/prisoner_population:
   *   get:
   *     summary: Retrieve data from Shahron Shaji ~
   *     tags: [Shahron Shaji ~]
   *     responses:
   ...
   *                   description: Error message
   */
  router.get('/sss135/prisoner_population', async (req, res) => {
    try {
      const result = await pool.query('SELECT * FROM prisoner_population WHERE owner = $1', ['sss135']);
      res.json(result.rows);
    } catch (err) {
  ...
  // Endpoint for Shahron Shaji ~: prisoner_security_class
  /**
   * @swagger
   * /sss135/prisoner_security_class:
   *   get:
   *     summary: Retrieve data from Shahron Shaji ~
   *     tags: [Shahron Shaji ~]
   *     responses:
   ...
   *                   description: Error message
   */
  router.get('/sss135/prisoner_security_class', async (req, res) => {
    try {
      const result = await pool.query('SELECT * FROM prisoner_security_class WHERE owner = $1', ['sss135']);
      res.json(result.rows);
    } catch (err) {
